...
SDK_DIR = arm-buildroot-linux-gnueabihf_sdk-buildroot
SDK_SYSROOT = $(SDK_DIR)/arm-buildroot-linux-gnueabihf/sysroot/
...
DFLAGS	= $(INCLUDE) -DRASPBERRY_PI -D_7ZIP_ST -DPACKAGE_VERSION=\"1.3.3\" -DUSE_SCHEDULER -DFLAC_API_EXPORTS -DFLAC__HAS_OGG=0 -DHAVE_LROUND -DHAVE_STDINT_H -DHAVE_STDLIB_H -DHAVE_SYS_PARAM_H -DENABLE_64_BIT_WORDS=0 -D_FILE_OFFSET_BITS=64 -D_LARGEFILE64_SOURCE -DVDATE=\"`date +"%y%m%d"`\"
...
clean:
	...
	$(Q)find . \( -name '*.o' -o -name '*.d' -o -name '*.bak' -o -name '*.rej' -o -name '*.org' \) | grep -v $(SDK_DIR) | xargs rm -vf
